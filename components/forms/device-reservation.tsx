...
import {
  availableTimesForDevice,
  createDeviceReservation,
  getDevices,
...
const formSchema = z.object({
  ...
  device: z.string({ required_error: "Please select a device." }),
  time: z.string(),
});
...
export default function DeviceBookingForm() {
  ...
  const {
    ...
    mutationFn: async ({
      ...
      await createDeviceReservation({ name, email, deviceName, date, time }),
  });
  ...
  return (
    <Form {...form}>
      <form
        ...
        className="space-y-8 border p-5 rounded-lg max-w-[500px] mx-auto"
      >
        <FormField
